...
internal class Setup
{
    ...
    {
        string[] riotProcesses = ["RiotClientServices", "LeagueClient", "VALORANT-Win64-Shipping"];
    ...
    internal static bool EnsureZombieVgcInstalled()
    {
        ...
        try
        {
            ...
            Console.WriteLine(" [INFO] Registering FakeFake.exe...");

            string exePath = Path.Combine(AppContext.BaseDirectory, "FakeVgc.exe");

            if (!File.Exists(exePath))
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine($" [ERROR] FakeVgc.exe not found in {exePath}");
                Console.ResetColor();
                return false;
            }
            ...
            if (serviceExists)
            {
                ...
                Console.WriteLine(" [SUCCESS] FakeVgc service successfully registered.");
                Console.ResetColor();
                return true;
            }
            else
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine(" [ERROR] Failed to register FakeVgc service.");
                Console.ResetColor();
                return false;
            }
        }
