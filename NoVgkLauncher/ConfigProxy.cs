...
public partial class ConfigProxy
{
    ...
    private static async Task HandleClientAsync(TcpClient client, CancellationToken token)
    {
        ...
        try
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                await ForwardServerToClientAsync(sslStream, clientStream, token);
            }
        }
        catch (Exception) {}
        finally
    ...
    private static async Task ForwardServerToClientAsync(Stream serverStream, Stream clientStream, CancellationToken token)
    {
        ...
        if (headerSection.Contains("Content-Encoding: gzip", StringComparison.OrdinalIgnoreCase))
        {
            ...
            decompressedBytes = ModifyResponsePayload(decompressedBytes);
        ...
        {
            byte[] modifiedBody = ModifyResponsePayload(bodyBytes);
    ...
    private static byte[] ModifyResponsePayload(byte[] payload)
    {
        string payloadStr = Encoding.UTF8.GetString(payload);
        var configObject = JsonSerializer.Deserialize<JsonNode>(payload);

        SetKey(configObject, "anticheat.vanguard.url", "");
        SetKey(configObject, "anticheat.vanguard.version", "1.0.0.0");
